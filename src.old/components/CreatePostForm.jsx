...
export default function CreatePostForm({ unidadesTerritoriales, stageDefs, defaultStageId, onCreated, onClose }) {
  const formKey = 'createpost';
  const saved = useMemo(() => getPersistedForm(formKey), []);
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      // Upload photos after post exists
      const uploadedUrls = [];
      ...
      if (uploadedUrls.length > 0 && selectedStageId) {
        ...
        await updateStageAtomic(newPost.id, selectedStageId, { photoUrl: uploadedUrls[0] });
