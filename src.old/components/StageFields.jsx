...
export function PhotoField({ photoReq, photoAdded, onToggle, color = '#F59E0B', onFileSelected }) {
  ...
  const handleFiles = (e) => {
    ...
    files.forEach(file => {
      ...
      reader.onload = (ev) => {
        setPreviews(prev => [...prev, { id: Date.now() + Math.random(), src: ev.target.result, file }]);
      };
      reader.readAsDataURL(file);
      // Enviar cada archivo al padre para upload
      ...
      {showEscalate && (
        <div className="p-4 border-2 border-red-500 bg-red-100 rounded-lg space-y-3">
          ...
          <div className="flex gap-2">
            ...
            <button onClick={() => {
              if (onCreateIncident) {
                onCreateIncident({ postId, type: incType || 'Incidencia general', description: incType || 'Sin descripción', severity: incSev, stageId: stage.id, sourceNote: notes?.trim() || '' });
              } else {
              ...
              <AlertCircle className="w-4 h-4" strokeWidth={2}/> Crear incidencia
            </button>
          </div>
        </div>
      )}
