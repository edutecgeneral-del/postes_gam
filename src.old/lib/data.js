...
function emptyStage() {
  return { done: false, ts: null, photo: null, capturedBy: null, verified: false, verifiedBy: null, verifiedAt: null, notes: '', attrs: {} };
}
...
export async function loadAllData() {
  ...
  const [postsData, stagesData, incidentsData, utsData] = await Promise.all([
    fetchAll('posts', 'id'),
...
export async function createPostAtomic({ direccion, lat, lng, unidad_territorial, zona_territorial, alias, numPoste, initialStageId, initialAttrs, initialNotes, initialPhotoUrl }) {
  const sb = requireSupabase();
  const { data, error } = await sb.rpc('create_post_atomic', {
  ...
  for (const sid of STAGE_IDS) {
    if (sid === initialStageId) {
      stages[sid] = { done: true, ts: Date.now(), photo: initialPhotoUrl || null, capturedBy: null, verified: false, verifiedBy: null, verifiedAt: null, notes: initialNotes || '', attrs: initialAttrs || {}, version: 1 };
    } else {
...
export async function updateStageAtomic(postId, stageId, { done, notes, attrs, photoUrl, needsScoutConfirm, expectedVersion }) {
  const sb = requireSupabase();
  const { data, error } = await sb.rpc('update_stage', {
    p_post_id: postId,
    p_stage_id: stageId,
    p_done: done ?? null,
    p_notes: notes ?? null,
...
export async function updatePostMetadata(postId, fields) {
  const sb = requireSupabase();
  const { data, error } = await sb.rpc('update_post_metadata', {
    p_post_id: postId,
...
export async function createIncidentAtomic({ postId, type, description, severity, stageId, sourceNote, blockPost, userNote, reportedByName }) {
  const sb = requireSupabase();
  const { data, error } = await sb.rpc('create_incident_atomic', {
    p_post_id: postId,
    p_type: type,
...
export async function resolveIncidentAtomic(incidentId) {
  const sb = requireSupabase();
  const { data, error } = await sb.rpc('resolve_incident_atomic', { p_incident_id: incidentId });
  if (error) throw error;
  return {
...
export async function savePost(post) {
  ...
  const stageRows = STAGE_IDS.map(sid => {
    ...
    return {
      ...
      photo_url: (typeof s.photo === 'string' && s.photo.startsWith('http')) ? s.photo : null,
...
export async function attendIncidentAtomic(incidentId, note, photoUrl) {
  const sb = requireSupabase();
  const { data, error } = await sb.rpc('attend_incident_atomic', {
...
export async function uploadIncidentPhoto(incidentId, file) {
  ...
  const filePath = `incidents/${incidentId}-${Date.now()}.${ext}`;
...
export async function uploadStagePhoto(postId, stageId, file) {
  const sb = requireSupabase();
  const ext = (file.name || '').split('.').pop() || 'jpg';
  const path = `${postId}/${stageId}-${Date.now()}.${ext}`;
...
export async function createScoutingVisit({ routeId, postId, gps, photo, generalResult, generalNotes, stageChecks }) {
  ...
  if (stageChecks && stageChecks.length > 0) {
    const rows = stageChecks.map(sc => ({
      visit_id: id,
      stage_id: STAGE_IDS.includes(sc.stageId) ? sc.stageId : 'marca',
